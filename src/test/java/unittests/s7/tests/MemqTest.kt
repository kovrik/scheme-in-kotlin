...
class MemqTest : AbstractTest() {
    ...
    @Test
    fun testMemq() {
        assertEquals(list<Symbol>(s("a"), s("b"), s("c")), eval("(memq 'a '(a b c))", env))
        assertEquals(list<Symbol>(s("a"), s("b"), s("c")), eval("(memq 'a (list 'a 'b 'c))", env))
        assertEquals(list<Symbol>(s("b"), s("c")), eval("(memq 'b '(a b c))", env))
        assertEquals(FALSE, eval("(memq 'a '(b c d))", env))
        assertEquals(FALSE, eval("(memq (list 'a) '(b (a) c))", env))
        assertEquals(list<Symbol>(s("a"), s("c"), s("a"), s("d"), s("a")), eval("(memq 'a '(b a c a d a))", env))
        ...
        assertEquals(cons<Symbol>(s("a"), s("b")), eval("(memq 'a (cons 'a 'b))", env))
        assertEquals(cons<Any>(s("a"), cons<Symbol>(s("b"), s("c"))), eval("(memq 'a '(a b . c))", env))
        assertEquals(cons<Symbol>(s("b"), s("c")), eval("(memq 'b '(a b . c))", env))
        assertEquals(list(Cons.EMPTY, 3L), eval("(memq '() '(1 () 3))", env))
        assertEquals(FALSE, eval("(memq '() '(1 2))", env))
        assertEquals(list<Symbol>(s("a"), s("b"), s("c")), eval("(memq 'a '(c d a b c))", env))
        assertEquals(FALSE, eval("(memq 'a '(c d f b c))", env))
        assertEquals(FALSE, eval("(memq 'a '())", env))
        assertEquals(cons<Any>(s("a"), cons<Symbol>(s("b"), s("c"))), eval("(memq 'a '(c d a b . c))", env))
        ...
        assertEquals(list<Symbol>(s("a"), s("a"), s("a")), eval("(memq 'a '(a a a))", env))
        assertEquals(list<Symbol>(s("a"), s("a")), eval("(memq 'a '(b a a))", env))
