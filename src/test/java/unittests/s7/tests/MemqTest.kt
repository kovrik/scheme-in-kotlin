...
class MemqTest : AbstractTest() {
    ...
    @Test
    fun testMemq() {
        assertEquals(listOf(s("a"), s("b"), s("c")), eval("(memq 'a '(a b c))", env))
        assertEquals(listOf(s("a"), s("b"), s("c")), eval("(memq 'a (list 'a 'b 'c))", env))
        assertEquals(listOf(s("b"), s("c")), eval("(memq 'b '(a b c))", env))
        assertEquals(false, eval("(memq 'a '(b c d))", env))
        assertEquals(false, eval("(memq (list 'a) '(b (a) c))", env))
        assertEquals(listOf(s("a"), s("c"), s("a"), s("d"), s("a")), eval("(memq 'a '(b a c a d a))", env))
        assertEquals(listOf<Any?>(false, 2L), eval("(memq #f '(1 a #t \"hi\" #f 2))", env))
        assertEquals(false, eval("(memq eq? (list 2 eqv? 2))", env))
        assertEquals(listOf(6L), eval("(memq 6 (memq 5 (memq 4 (memq 3 (memq 2 (memq 1 '(1 2 3 4 5 6)))))))", env))
        ...
        assertEquals(listOf(emptyList<Nothing>(), 3L), eval("(memq '() '(1 () 3))", env))
        assertEquals(false, eval("(memq '() '(1 2))", env))
        assertEquals(listOf(s("a"), s("b"), s("c")), eval("(memq 'a '(c d a b c))", env))
        ...
        assertEquals(listOf(listOf<Any>() as Any), eval("(memq '() (list '()))", env))
        assertEquals(listOf(cons(1L, 2L), cons(3L, 4L)), eval("(let ((x (cons 1 2))) (memq x (list x (cons 3 4))))", env))
        assertEquals(true, eval("(pair? (let ((x (lambda () 1))) (memq x (list 1 2 x 3))))", env))
        assertEquals(listOf(s("a"), s("a"), s("a")), eval("(memq 'a '(a a a))", env))
        assertEquals(listOf(s("a"), s("a")), eval("(memq 'a '(b a a))", env))
        assertEquals(listOf<Any?>("hi", 2L), eval("(memq \"hi\" '(1 \"hi\" 2))", env))
        assertEquals(listOf<Any?>('a', 2L), eval("(memq #\\a '(1 #f #\\a 2))", env))
        try {
