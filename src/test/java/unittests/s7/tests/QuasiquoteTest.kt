...
class QuasiquoteTest : AbstractTest() {
    ...
    fun testQuasiquote() {
        ...
        assertEquals(list(s("a"), list(s("quasiquote"), list(s("b"), list<Symbol>(s("unquote"), s("x")), list<Any>(s("unquote"), list<Symbol>(s("quote"), s("y"))), s("d"))), s("e")),
        ...
        assertEquals(list<Symbol>(s("+")), eval("(let ((x '())) `(+ ,@x)) '(+)", env))
