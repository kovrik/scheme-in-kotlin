...
class EqualTest : AbstractTest() {
    ...
    fun testEqual() {
        ...
        val trues = arrayOf(
                "(equal? #(a) #(a))", "(equal? '(1) '(1))", "(equal? '(#f) '(#f))", "(equal? '(()) '(() . ()))",
                "(equal? #\\a #\\a)", "(let ((x (string-ref \"hi\" 0))) (equal? x x))", "(equal? #t #t)", "(equal? #f #f)",
                "(equal? (null? null) #t)", "(equal? (null? '(a)) #f)", "(equal? (cdr '(a)) '())", "(equal? 'a 'a)",
                "(equal? '(a) '(a))", "(let ((x '(a . b))) (equal? x x))", "(let ((x (cons 'a 'b))) (equal? x x))",
                "(equal? (cons 'a 'b) (cons 'a 'b))", "(equal?(cons 'a 'b)(cons 'a 'b))", "(equal? \"abc\" \"abc\")",
        ...
        val falses = arrayOf(
                ...
                "(equal? \"hi\" '(hi))", "(equal? \"()\" '())", "(equal? #\\a #\\b)", "(equal? #f #t)", "(equal? 'a 'b)",
                "(equal? '(a) '(b))", "(equal? \"abc\" \"cba\")", "(equal? #(a) #(b))", "(equal? #(1 2) (vector 1 2.0))",
                "(equal? #(1) #('1))", "(equal? car cdr)", "(equal? (lambda () 1) (lambda () 1))", "(equal? '(1) '(list 1))",
