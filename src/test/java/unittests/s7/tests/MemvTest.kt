...
class MemvTest : AbstractTest() {
    ...
    fun testMemv() {
        ...
        assertEquals(listOf(101L, 102L), eval("(memv 101 (list 100 101 102))", env))
        assertEquals(listOf(3.4, 4.5), eval("(memv 3.4 '(1.2 2.3 3.4 4.5))", env))
        ...
        assertEquals(listOf(s("c")), eval("(memv 'c '(a b c))", env))
        assertEquals(false, eval("(memv ''a '('a b c))", env))
        assertEquals(false, eval("(let ((x (cons 1 2))) (memv x (list (cons 1 2) (cons 3 4))))", env))
        assertEquals(listOf(cons(1L, 2L), cons(3L, 4L)), eval("(let ((x (cons 1 2))) (memv x (list x (cons 3 4))))", env))
        assertEquals(listOf(s("a"), s("a"), s("a")), eval("(memv 'a '(a a a))", env))
        assertEquals(listOf(s("a"), s("a")), eval("(memv 'a '(b a a))", env))
        assertEquals(listOf<Any?>("hi", 2L), eval("(memv \"hi\" '(1 \"hi\" 2))", env))
        assertEquals(listOf<Any?>('a', 2L), eval("(memv #\\a '(1 #f #\\a 2))", env))
        assertEquals(false, eval("(memv #(1) '(1 #(1) 2))", env))
        assertEquals(listOf<Any?>(emptyList<Nothing>(), 2L), eval("(memv '() '(1 () 2))", env))
        assertEquals(listOf(MutableVector(arrayOf(1L, 2L, 3L)), MutableVector(arrayOf(1L, 2L))), eval("(let* ((x (vector 1 2 3)) (lst (list 1 \"hi\" x (vector 1 2)))) (memv x lst))", env))
