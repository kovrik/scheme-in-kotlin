...
public class Assoc extends AFn {
  ...
  public Object invoke(Object... args) {
    if (args != null && args.length == 2) {
      if (!(args[1] instanceof List)) {
        throw new IllegalArgumentException(
            String.format("Wrong type argument to `assoc`! Expected: List, Actual: %s", args[1]));
      }
      Object obj = args[0];
      ...
      for (int n = 0, listSize = list.size(); n < listSize; n++) {
        ...
        if (SCMCons.isPair(pair)) {
          if (((SCMCons)pair).car().equals(obj)) {
            return pair;
          }
        } else {
          throw new IllegalArgumentException(
    ...
    throw new ArityException(args.length, 2, "assoc");
  }
