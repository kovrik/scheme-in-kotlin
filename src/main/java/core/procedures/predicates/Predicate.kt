...
class Predicate private constructor(override val name: String, private val predicate: java.util.function.Predicate<Any?>) : AFn(FnArgsBuilder().min(1).max(1).build()) {
    ...
    companion object {
        ...
        val IS_EVEN = Predicate("even?", java.util.function.Predicate<Any?> { o -> assertClass("even?", o, Int::class.javaObjectType) && Utils.isZero(Remainder.apply(o as Number, 2L)) })
        val IS_ODD = Predicate("odd?", java.util.function.Predicate<Any?> { o -> assertClass("odd?", o, Int::class.javaObjectType) && !Utils.isZero(Remainder.apply(o as Number, 2L)) })
